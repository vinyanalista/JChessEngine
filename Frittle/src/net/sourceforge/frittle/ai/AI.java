...
import br.com.vinyanalista.jchessengine.XBoard;
import br.com.vinyanalista.jchessengine.example.Frittle;
...
public class AI
{
	...
	protected final Frittle engine;
	...
	public AI(Frittle engine)
	{
        ...
        try
        {
            ...
            engine.debug("Initializing hashtable...");
            ...
            engine.debug("Hashtable ready [" + positions + " million positions in " + hashtableMB + "MB]");
            engine.debug("Loading book...");
            book = new Book("tiny.book");
            long afterBook = runtime.totalMemory()-runtime.freeMemory();
            int bookMB = (int)(afterBook-afterHash)/(1024*1024);
            engine.debug("Book loaded [" + book.size() + " positions in " + bookMB + "MB]");
        }
        catch(java.io.FileNotFoundException e)
        {
            engine.debug("ERROR - Could not find book file!!!");
        }
        catch(BookFormatException e)
        {
            engine.debug("ERROR - " + e.getMessage());
        }
        catch(java.io.IOException e)
        {
            engine.debug(e.toString());
        }
        ...
        clock = new Clock(engine.getGame().getClockFormat().getBaseTime());
	}
	...
	public void go()
	{
		...
		GameState state = engine.getGame().getCurrentState();
        // Check book
        ...
        if(bookMoveStr != null)
        {
            ...
            if(engine.getGame().doMove(bookMoveStr))
                XBoard.move(bookMoveStr);
            else
                XBoard.tellUserError("Illegal move in book");
        ...
        else // There was no ponder or the opponent did not play the expected move
        {
            ...
            search = new Search(this, state, this.searchDepth, searchTime, engine);
            search.start();
        ...
        if(search != null && !search.isPonder()) // Ponder searches aren't active unless we go()
        {
            ...
            else
            {
                ...
                if( bestMove != null )
                {
                    ...
                    XBoard.move(bestMove);
                    engine.getGame().doMove(bestMove); // This might return false if user interrupted game state
                }
                else
                {
                	XBoard.tellUserError("Nothing to play");
                }
                // Terminate the search thread and pending timer tasks
                this.destroyThreads();
    ...
    private void ponder()
    {
        ...
        this.search = new Search(this, state, this.searchDepth, -1, engine);
        ...
        // Announce it
        engine.debug("Pondering on " + expectedReply.toString());
    ...
    {
        int movesPerSession = engine.getGame().getClockFormat().getMovesPerSession();
        if(movesPerSession > 0) // Tournament time
        {
            // No. of moves elapsed = Move number - 1 (because move number is for next move)
            int elapsedMoves = engine.getGame().getMoveNumber()-1;
        ...
        {
            long increment = engine.getGame().getClockFormat().getIncrement();
